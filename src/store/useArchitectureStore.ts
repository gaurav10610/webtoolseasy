...
type ArchitectureState = {
  ...
  onConnect: OnConnect;
  addNode: (service: InfraNodeData['service'], position: { x: number; y: number }) => void;
...
const defaultNodes: InfraNode[] = [
  {
    ...
    data: { label: 'Application Load Balancer', service: 'ALB', region: 'us-east-1', config: { hoursPerMonth: 730, lcuCount: 1 }, costPerMonth: 22.26 },
  },
  {
    id: 'ec2-1',
    type: 'infraNode',
    position: { x: 300, y: 300 },
    data: { label: 'Web Server 1', service: 'EC2', region: 'us-east-1', config: { instanceType: 't3.micro', count: 1, hoursPerMonth: 730 }, costPerMonth: 7.59 },
  },
  {
    id: 'ec2-2',
    type: 'infraNode',
    position: { x: 500, y: 300 },
    data: { label: 'Web Server 2', service: 'EC2', region: 'us-east-1', config: { instanceType: 't3.micro', count: 1, hoursPerMonth: 730 }, costPerMonth: 7.59 },
  },
  {
    id: 'rds-1',
    type: 'infraNode',
    position: { x: 400, y: 500 },
    data: { label: 'Primary Database', service: 'RDS', region: 'us-east-1', config: { instanceType: 'db.t3.micro', storageGB: 20, multiAZ: true }, costPerMonth: 27.12 },
  },
];
...
function recalculateNodeCost(node: InfraNode): InfraNode {
  return {
    ...
    data: {
      ...node.data,
      costPerMonth: calculateNodeCost(node.data.service, node.data.config),
    }
  };
}
...
export const useArchitectureStore = create<ArchitectureState>((set, get) => ({
  ...
  addNode: (service: InfraNodeData['service'], position: { x: number; y: number }) => {
    const id = `${service}-${Date.now()}`;
    // Default configs
    let config = {};
    if (service === 'EC2') config = { instanceType: 't3.micro', count: 1, hoursPerMonth: 730 };
    if (service === 'RDS') config = { instanceType: 'db.t3.micro', storageGB: 20, multiAZ: false };
    if (service === 'S3') config = { storageGB: 100, readReqsMillions: 0, writeReqsMillions: 0 };
    if (service === 'Lambda') config = { invocationsMillions: 1, memoryMB: 128, durationMs: 100 };
    ...
    const newNode: InfraNode = {
      ...
      data: {
        ...
        region: 'us-east-1',
        config,
        costPerMonth: calculateNodeCost(service, config),
      },
    };
    ...
    const updated = get().nodes.map((node) => {
      ...
      return {
        ...
        data: {
          ...node.data,
          config: newConfig,
          costPerMonth: calculateNodeCost(node.data.service, newConfig),
        }
      };
    });
    set({ nodes: updated });
  ...
  getTotalCost: () => {
    return get().nodes.reduce((total, node) => total + (node.data.costPerMonth || 0), 0);
  },
}));
