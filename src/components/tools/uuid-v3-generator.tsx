...
export default function UuidV3Generator({
  ...
  const generateUuidV3 = useCallback(
    (inputName: string) => {
      const namespaceUuid =
        namespace === "CUSTOM"
          ? customNamespace
          : NAMESPACES[namespace as keyof typeof NAMESPACES];
      return uuidv3(inputName, namespaceUuid);
    },
    [namespace, customNamespace]
  );

  const currentUuid = name ? generateUuidV3(name) : "";
  ...
  const generateNewUuid = useCallback(() => {
    if (!name) {
      toolState.actions.showMessage("Please enter a name to generate UUID v3");
      return;
    }
    ...
    try {
      const newUuid = generateUuidV3(name);
      ...
      toolState.actions.showMessage("UUID v3 generated!");
    } catch {
      toolState.actions.showMessage(
        "Error generating UUID. Check your inputs."
      );
    }
  }, [name, namespace, customNamespace, generateUuidV3, toolState]);
  ...
  const generateBulkUuids = useCallback(() => {
    ...
    try {
      ...
      const newUuids = Array.from({ length: count }, (_, i) =>
        generateUuidV3(`${bulkNamePrefix}${i + 1}`)
      );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
                ...
                {uuidList.map((uuid, index) => (
                  <div key={index} className="p-3 font-mono text-sm">
                    <div className="flex items-center justify-between">
                      <span className="text-orange-600">{uuid}</span>
          ...
          <div className="p-4 bg-yellow-50 border border-yellow-300 rounded">
            <Typography variant="subtitle2" className="mb-1 text-yellow-800">
              ⚠️ Legacy Standard
            </Typography>
            <Typography variant="caption" className="text-yellow-700">
              UUID v3 uses MD5 hashing. For new projects, prefer UUID v5 (SHA-1)
