...
function FlowInner() {
  const { nodes, edges, onNodesChange, onEdgesChange, onConnect, addNode } = useArchitectureStore();
  ...
  const onDrop = useCallback(
    (event: React.DragEvent) => {
      event.preventDefault();
      const service = event.dataTransfer.getData('application/reactflow/service') as InfraService;
      if (!service) return;
  ...
  return (
    <div className="flex-1 relative" ref={reactFlowWrapper}>
      {nodes.length === 0 && (
        <div className="absolute inset-0 pointer-events-none flex items-center justify-center z-10 opacity-30">
          <div className="text-center">
            ...
            <p className="text-gray-500 font-medium">Drag AWS resources here to build architecture</p>
          </div>
        </div>
      )}
      ...
      <ReactFlow
        ...
        colorMode="dark"
        className="bg-[#0A0A0B]"
        defaultEdgeOptions={{ style: { stroke: '#6366f1', strokeWidth: 2 }, type: 'smoothstep' }}
      >
...
export function ArchitectureCanvas({ initialTemplate }: ArchitectureCanvasProps = {}) {
      ...
      <header className="h-16 bg-[#0A0A0B]/90 backdrop-blur-md border-b border-white/10 px-6 flex justify-between items-center shrink-0 z-20">
        <div className="flex items-center gap-4">
          <a href="/" className="flex items-center gap-3 no-underline group">
            ...
            <div>
              <span className="text-lg font-bold text-white tracking-tight block">ArchCost</span>
              <span className="text-[10px] text-orange-400 font-mono tracking-widest uppercase block mt-[-2px]">Visual Cloud Estimator</span>
            </div>
          </a>
        </div>
        ...
        <div className="flex gap-2">
          ...
          <button
            className="bg-indigo-600 hover:bg-indigo-500 shadow-[0_0_20px_rgba(79,70,229,0.3)] text-white px-5 py-2 rounded-lg font-semibold transition-all flex items-center gap-2 text-sm ml-1"
          >
