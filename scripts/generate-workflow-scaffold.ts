 ...
 * Usage: node scripts/generate-workflow-scaffold.ts <workflow-name> [category]
 * Example: node scripts/generate-workflow-scaffold.ts "api-payload-cleanup" "developer"
 */
...
function validateInputs(args: string[]): WorkflowOptions {
  if (args.length < 2) {
    console.error(
      "Usage: npx ts-node scripts/generate-workflow-scaffold.ts <name> [category]",
    );
...
function generateConfigTemplate(opts: WorkflowOptions): string {
  return `{
    id: "wf-${opts.slug}",
    slug: "${opts.slug}",
...
function generateStepComponentTemplate(
...
function generateUnitTestTemplate(opts: WorkflowOptions): string {
...
function generateE2ETestTemplate(opts: WorkflowOptions): string {
...
function createScaffold(opts: WorkflowOptions): void {
